...
*/
// 弹窗提示，表示代码进入了if分支
  $notify("调试信息", "执行成功", "已修改 has_premium_access 为 1");
if ($request.url.indexOf("lcs-mobile-cops.adobe.io") != -1) {
  // 修改第一个请求的body
// 弹窗提示，表示代码进入了if分支
  $notify("调试信息", "执行成功", "已修改 has_premium_access 为 2");
  var objc1 = {
    "asnp": {
  ...
  // 修改第二个请求的body
// 弹窗提示，表示代码进入了if分支
  $notify("调试信息", "执行成功", "已修改 has_premium_access 为 3");
  var objc = {
    "user" : {
...
$done({ body: JSON.stringify(objc) });
  ...
  // 弹窗提示，表示代码进入了if分支
  $notify("调试信息", "执行成功", "已修改 has_premium_access 为 4");
